...
#
# 🔴 LA LIBRETA LA ESCRIBEN DOS USUARIOS y el que la crea decide si el otro puede.
# Apache es www-data (ahi entra la pulsacion del vendedor); este bucle es root.
# El 9-sep-2026 quedo root:root 0644 y Apache recibio "attempt to write a readonly
# database": la cola marcaba 0 con pulsaciones reales entrando.
#
# ⚠ SE ARREGLA CON LOS PERMISOS DEL ARCHIVO, NO CAMBIANDO EL USUARIO DEL BUCLE.
# Probe correrlo con `su -s /bin/sh www-data` y quedo PEOR, medido en el log:
#   /bin/sh: 1: php: not found          <- `su` no hereda el PATH con /usr/local/bin
#   .: cannot open /data/env.sh          <- env.sh es 600 de root, www-data no lo lee
# O sea que para ganar un usuario habia que abrir el archivo de credenciales y
# arreglar el PATH: dos puntos de fallo nuevos. Con el archivo en 0666 da igual
# quien lo cree primero, y cola_nc_db() lo deja asi siempre.
